...
根据机器人的起点、终点、运动模型和环境信息等参数，计算出一条合适的路径或轨迹来实现特定的任务，这样的问题便是机器人学中的运动规划问题。
...
挪钢琴问题顾名思义，是一个在房间里移动钢琴的问题：假设我们有一架钢琴的三维模型和整个房子的三维模型，我们希望寻找一条*轨迹*，将钢琴从一个位置，经过走廊、楼梯、或许相对狭窄的门而不发生碰撞，搬到另一个指定房间的指定位置。

我们需要明确“自由度”的概念。在运动学中，自由度表示一个物体在空间中可以自由运动的方向和程度。例如，一维自由度表示物体可以在一个方向上运动，二维自由度表示物体可以在平面上自由运动，三维自由度表示物体可以在三维空间中自由运动。而对于钢琴这样的三维物体，它的运动自由度就比较复杂，一般来说，它会有六个自由度，分别是：在三个方向上的平移自由度和绕三个轴的旋转自由度。

在钢琴的运动规划问题中，我们需要考虑到钢琴的位置和旋转角度，这就相当于在一个六维空间中进行规划。这个六维空间中的每一个点，都代表着钢琴的一个位置和旋转角度。
...
所有的规划问题都需要在*状态空间*(state space)上进行。所谓状态空间，指的是在一个规划问题中，所有可能达到的状态(state)或情形。
...
我们既可以显式地表达时间，即规划结果的每一个状态都有一个对应的时间；而当具体的时间没有那么重要，只需要按照顺序执行每一个行动时，我们也可以隐式地表示时间，例如华容道只需要记录每一步该怎么走，而不需要关注第几秒走第几步。

我们也可以选择在时间上离散地表达一条轨迹，或者连续地表达轨迹。离散的表示法，就是把整个轨迹分成若干个小段，每一段的起点和终点都用一个点来表示，这些点构成了轨迹的离散表示。这种表示法的优势是简单明了，易于计算，缺点是轨迹的精度受到限制、两个点之间的状态还需要插值计算。连续的函数表示法，就是用一个函数来表示整个轨迹，这个函数可以是一条曲线、多条首尾相连的分段曲线等。这种表示法的优势是可以得到精确和平滑的轨迹，可以通过函数得到任意时间点的状态，缺点是轨迹的参数化可能并不直观、计算和优化过程比较复杂。

对于运动规划的问题，有时我们也可以隐式地表达时间，比如挪钢琴问题中我们可以只记录钢琴经历过的所有状态，而不记录经过各个状态的时间；这样在后续步骤中，我们可以通过改变各个状态的时间戳控制挪动过程的快慢。
